...
fn test_into_hashmap() {
    let val = NewType::Var(Some(Inner { attr: "foo".into() }));
    ...
    let val = NewType::Var(None);
    let actual: HashMap<String, AttributeValue> = val.into();

    let expected: HashMap<String, AttributeValue> =
        [("Var".to_string(), AttributeValue::Null(true))].into();
...
fn test_try_from_hashmap() {
    let expected = NewType::Var(Some(Inner { attr: "foo".into() }));
